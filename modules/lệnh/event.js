mô-đun . hàng xuất khẩu . config  =  {
	...
	tập quán : "[tải / dỡ / tảiAll / dỡ-tải / thông tin] [tên mô-đun]" ,
...
mô-đun . hàng xuất khẩu . run  =  function  ( { event , args , api , getText } )  {
    const  { readdirSync }  =  toàn cầu . nốt sần [ "fs-extra" ] ;
    const  { tham gia }  =  toàn cầu . nốt sần [ "đường dẫn" ] ;
    const  { threadID , messageID }  =  sự kiện ;
    var  moduleList  =  args . mối nối ( 1 ,  chiều dài args . ) ;

    chuyển đổi  ( args [ 0 ] )  {
        trường hợp  "tải" : {
            if  ( moduleList . length  ==  0 )  trả về  api . sendMessage ( getText ( "missingInput" ) ,  threadID ,  messageID ) ;
            khác  trả lại  cái này . loadCommand ( { moduleList , threadID , messageID , getText } ) ;
        }
        trường hợp  "dỡ hàng" : {
            if  ( moduleList . length  ==  0 )  trả về  api . sendMessage ( getText ( "missingInput" ) ,  threadID ,  messageID ) ;
            khác  trả lại  cái này . unloadModule ( { moduleList , threadID , messageID , getText } ) ;
        }
        trường hợp  "loadAll" : {
            moduleList  =  readdirSync ( tham gia ( global . client . mainPath ,  "modules" ,  "event" ) ) . filter ( ( file )  =>  file . endWith ( ".js" )  &&  ! file . include ( 'example' ) ) ;
            moduleList  =  moduleList . map ( item  =>  item . Replace ( / \. js / g ,  "" ) ) ;
            trả lại  cái này . loadCommand ( { moduleList , threadID , messageID , getText } ) ;
        }
        trường hợp  "dỡ bỏ tất cả" : {
            moduleList  =  readdirSync ( tham gia ( global . client . mainPath ,  "modules" ,  "event" ) ) . filter ( ( file )  =>  file . endWith ( ".js" )  &&  ! file . include ( 'example' ) ) ;
            moduleList  =  moduleList . map ( item  =>  item . Replace ( / \. js / g ,  "" ) ) ;
            trả lại  cái này . unloadModule ( { moduleList , threadID , messageID , getText } ) ;
        }
        trường hợp  "thông tin" : {
             sự kiện  const =  toàn cục . khách hàng . các sự kiện . get ( moduleList . join ( "" )  ||  "" ) ;
            if  ( ! event )  trả về  api . sendMessage ( getText ( "moduleNotExist" ) ,  threadID ,  messageID ) ;
            const  { tên , phiên bản , tín dụng , phụ thuộc }  =  sự kiện . cấu hình ;
            trả lại  api . sendMessage ( getText ( "infoModule" ,  tên . toUpperCase ( ) ,  tín dụng ,  phiên bản ,  ( ( Đối tượng . khóa ( phụ thuộc  ||  { } ) ) . join ( "," )  ||  getText ( "dontHavePackage" ) ) ) ,  threadID ,  messageID ) ;
        }
        mặc định : {
            trở lại  toàn cầu . utils . némError ( this . config . name ,  threadID ,  messageID ) ;
        }
    }
}
Chân trang
